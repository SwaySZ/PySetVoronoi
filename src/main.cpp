...
//			if(j == i) continue;//
			...
			if(parAttrlist.at(i).radius * radiation_radius > dist ){
				//p2 close to p1
				parAttrlist.at(i).surroundedID.push_back(j); // find all particle close to p1 not including current particle itself;
			}
...
std::map < unsigned int, double> cellVolumes;//
std::map < unsigned int,double > cellSurfaceArea;//surface area of each cell
std::map < unsigned int,double> particleVolumes;//
std::map < unsigned int,double> particleSurfaceArea;//
std::map < unsigned int, std::vector<double> > cellNormalTensor;//six components of normal tensor of a cell surface, i.e., n11,n12,n13,n22,n23,n33.
std::map < unsigned int, std::vector<double> > cellNormalAreaTensor;//six components of normal by area tensor of a cell surface, i.e., n11*a,n12*a,n13*a,n22*a,n23*a,n33*a.
fileloader loader;
...
//we need to know xmin, xmax, ymin, ymax, zmin, zmax, nx, ny, nz, xpbc, ypbc, zpbc, 32
 ...
 for(unsigned int id = 0; id <parAttrlist.size();id++){
	 ...
	 std::map < unsigned int, unsigned int> labelidmap;
	 ...
	 for(unsigned int j = 0 ;j<surroundedID.size();j++){
		 ...
		 pointpattern pp = pplist.at(particleID);
		               ...
		               ++it)
		       {
		           ...
		           unsigned int l = it->l;
//		           if(l == particleID){
//		        	   std::cout<<"l == particle ID"<<std::endl;
//		           }else{
//		        	   std::cout<< "l: "<<l<<"particle ID"<<particleID<<std::endl;
//		           }
		           labelidmap[pid] = l;
		           ++pid;
		       }
	 ...
	 // cell currently worked on
	  ...
	  if(cla.start())
	  {
		  std::cout <<"current particle ID: "<<currentparticleID<< "  started\n" << std::flush;
		         do
		         {
	  ...
	  pwlist.push_back(pw);
	  std::cout<<pw.p.points.size()<<std::endl;
	  std::cout<<std::endl;
...
//		){
    ...
    //go througn all the cell parameters
    for(std::map < unsigned int, double >::iterator  iter = cellVolumes.begin(); iter != cellVolumes.end(); iter++) {
        // iter->first = key
        // iter->second = value
        if(state["RawData"]){
            //to do
	...
	if(state["isBatch"] == false)//output polydata when running one job
	{
		...
		{	std::string filename = folder + "/reduced"+std::to_string(i)+".xyz";
		    pw.savePointPatternForGnuplot(filename);
		}
		...
		if(state["cellVTK"] == true){
			std::cout<<"writing vtk file" <<std::endl;
				std::string filename = folder + "/cell"+std::to_string(i)+".vtk";
				pw.saveOnePolyVTK(filename);
			}
		//write pov file
		if(state["savepov"] == true)
		{
			double rr = state["rr"];
			std::string filename = folder + "/cell" + std::to_string(i)+ ".pov";
		    pw.savePolyPOV(filename,xmin,xmax,ymin,ymax,zmin,zmax,rr);
		}
...
//		    std::cout << "writing poly file" << std::endl;
		    std::ofstream file;
		    std::string polypath =folder + "/cell"+std::to_string(i)+".poly";
