...
//        std::cout << "writing Polydata into a VTK file" << std::endl;
    ...
    void saveOnePolyVTK(std::string filename)
    {
        std::cout << "writing Polydata into a VTK file" << std::endl;
        std::ofstream f;
        f.open(filename.c_str(),std::ios::out);
        ...
        std::vector<unsigned long> polydata;

		unsigned long numPoints = 0;
        unsigned int face_num = 0, enterty_num = 0;
        for (
            auto it = this->faces.begin();
            it != this->faces.end();
            ++ it)
    ...
    void outputOneCellVTK(std::string filename,unsigned int targetCellID){
    	        std::cout << "writing Polydata into a VTK file" << std::endl;
    	        std::ofstream f;
    	        f.open(filename.c_str(),std::ios::out);
    			//store points and polygons
